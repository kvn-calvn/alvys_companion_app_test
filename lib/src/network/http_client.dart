...
class AlvysHttpClient {
  ...
  Future<Response> getData<T>(Uri uri, {Map<String, String>? headers}) {
    return _executeRequest<T>(
        () async => telemetryHttpClient.get(uri, headers: getHeaders(headers)));
  }

  Future<Response> postData<T>(Uri uri,
      {Map<String, String>? headers, Object? body, Encoding? encoding}) {
    return _executeRequest<T>(() async => telemetryHttpClient.post(uri,
        headers: getHeaders(headers), body: body, encoding: encoding));
  }
  ...
  Future<void> addPermissionDetails() async {
    ...
    telemetryClient.context.properties['permissionStatus'] = Map.fromEntries(
            await permissions.mapAsync((element) async => MapEntry(
                element.toString().replaceAll('Permission.', ''),
                (await element.status).name)))
        .toJsonEncodedString;
  }
